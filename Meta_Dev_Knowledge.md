...
- R-04（2026-09-11全倉庫稽核發現，尚未觸發，已加CI防護）：同一級別內若 `de`
  重複，`js/app.js` 的 `cardId()`（`level + ":" + de`）會讓兩個不同單字共用
  同一份 SRS 進度，其中一個的複習狀態被另一個悄悄覆蓋；若 `zh` 重複，
  `buildQuiz()` 用文字比對判斷測驗選項對錯，會出現兩顆看起來一樣的選項。
